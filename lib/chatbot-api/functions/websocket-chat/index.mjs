  ...
  try {
    ...
    if (doNotSave) {
      ...
      if (amendments.length > 0) {
        ...
        SYS_PROMPT = `
          You are an AI assistant that will look through a list of session law excerpts for amendments to Chapter ${chapter} of the Acts of ${year}. You will carefully go through each excerpt to generate a list of
          amendments to the specific Act. You will include every single relevant amendment without skipping any. Err on the side of inclusion if uncertain, but avoid known false positives wherever possible.
          The most common false positive is when you are looking for amendments to Chapter X but get an excerpt for Section X of some other chapter. These instances should not be included.
          
          You will use the find_references tool to get the amendment data and then
          the send_amendments_to_client tool to send a structured, summarized list of those amendments with as many details as possible. If the same act makes multiple amendments, please group them together.          
        `
      }
    }
