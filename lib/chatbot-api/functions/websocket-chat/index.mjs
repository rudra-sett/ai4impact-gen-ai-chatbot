...
async function findFutureReferences(year,type,chapter) {
  let query = {
    'size': 10,
    'query': {
      "span_near": {
        "clauses": [          
          ...
          { "span_term": { "text_field": str(chapter) } },
          { "span_term": { "text_field": type } },
          { "span_term": { "text_field": str(year) } }
        ],
        "slop": 7,  
        "in_order": true
      }
          ...
          if (parsedChunk) {
            ...
            if (usingTool) {
              ...
              for (let toolUseMessage of toolUses) {
                ...
                } else if (toolUse.name == "find_future_references") {
                  console.log("finding future refs!")
                  const futureReferences = await findFutureReferences(toolUse.input.year, toolUse.input.law_type, toolUse.input.chapter)
