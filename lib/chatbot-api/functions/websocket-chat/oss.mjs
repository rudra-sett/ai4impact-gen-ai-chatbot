...
export async function search(query) {
  ...
  if (hits.length < 124) {
    searchResult = hits.map((result,index) => {
      return `<amendment_data index="${index}"><amending_act_file>` + result._source['x-amz-bedrock-kb-source-uri'] + "</amending_act_file><amendment_chunk_content>" + result._source.text_field + "</amendment_chunk_content></amendment_data>"
    })
  } else {
